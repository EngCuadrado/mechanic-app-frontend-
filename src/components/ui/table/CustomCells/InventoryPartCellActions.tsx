...
import EditInventoryPart from "../CustomDrawers/EditInventoryPart";
import { TOGGLE_INVENTORY_PART_STATUS_MUTATION } from "../QuerysDefinitions";
...
interface EditInventoryPartDrawerProps {
	row: {
		original: InventoryPartData;
	};
	onClose: () => void;
}
...
interface MenuItemType {
	...
	drawer: React.ComponentType<{ row: { original: InventoryPartData }; onClose: () => void }>;
}

interface InventoryPartCellActionsProps {
	row: {
		original: InventoryPartData;
	};
}

const EditInventoryPartModal = ({ row, onClose }: EditInventoryPartDrawerProps) => {
	const intl = useIntl();
	...
	return (
		<SimpleModal
			isOpen={true}
			onClose={onClose}
			title={intl.formatMessage({ id: "inventory.edit", defaultMessage: "Editar Producto" })}
			widthClass="w-full max-w-md md:max-w-xl"
			disableOutsideClick={true}
		>
			<EditInventoryPart row={row} onClose={onClose} />
		</SimpleModal>
	);
};
...
const ToggleActivatedModal = ({ row, onClose }: ToggleActivatedModalProps) => {
	const intl = useIntl();
	const { inventoryPartId, isActive, name } = row.original;

	const [toggleStatus, { loading }] = useMutation(
		TOGGLE_INVENTORY_PART_STATUS_MUTATION,
		{
	...
	const handleConfirm = () => {
		toggleStatus({
			variables: {
				inventoryPartId: Number(inventoryPartId),
			},
		});
	};
...
export const InventoryPartCellActions = ({ row }: InventoryPartCellActionsProps) => {
	const { original } = row;

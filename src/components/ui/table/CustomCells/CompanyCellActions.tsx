...
import EditCompany from "../CustomDrawers/EditCompany";
import { TOGGLE_COMPANY_STATUS_MUTATION, GET_COMPANIES_QUERY } from "../QuerysDefinitions";
...
interface MenuItemProps {
	...
	drawer: React.ComponentType<{ row: { original: CompanyData }; onClose: () => void }>;
}

interface CompanyCellActionsProps {
	row: {
		original: CompanyData;
	};
}

const EditCompanyModal = ({ row, onClose }: EditCompanyModalProps) => {
	const intl = useIntl();
	...
	return (
		<SimpleModal
			isOpen={true}
			onClose={onClose}
			title={intl.formatMessage({ id: "company.edit", defaultMessage: "Editar Empresa" })}
			widthClass="w-full max-w-lg md:max-w-2xl lg:max-w-3xl"
			disableOutsideClick={true}
		>
			<EditCompany row={row} onClose={onClose} />
		</SimpleModal>
	);
};
...
const ToggleActivatedModal = ({ row, onClose }: ToggleActivatedModalProps) => {
	const intl = useIntl();
	const { companyId, isActive, name } = row.original;

	const [toggleStatus, { loading }] = useMutation(
		TOGGLE_COMPANY_STATUS_MUTATION,
		{
...
export const CompanyCellActions = ({ row }: CompanyCellActionsProps) => {
	const { original } = row;
